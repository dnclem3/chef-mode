...
def handler(event, context):
    ...
    print('🐍 Python extraction handler called')
    
    try:
        # Parse request body
        body = json.loads(event.get('body', '{}'))
        print('📥 Request body:', {
            'hasUrl': bool(body.get('url')),
            'hasHtml': bool(body.get('html')),
            'urlLength': len(body.get('url', '')),
            'htmlLength': len(body.get('html', ''))
        })
        ...
        return {
            'statusCode': 200 if result['success'] else 422,
            'headers': {
